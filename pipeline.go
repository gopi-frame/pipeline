...
var _ pipeline.Pipeline[any, any] = (*Pipeline[any, any])(nil)

// Pipeline pipeline
type Pipeline[P, R any] struct {
	passable    P
	stops       []pipeline.Stop[P, R]
...
func (p *Pipeline[P, R]) Through(stops ...pipeline.Stop[P, R]) pipeline.Pipeline[P, R] {
	p.stops = stops
	return p
}
...
func (p *Pipeline[P, R]) Then(destination func(P) R) R {
	...
	for i := length; i > 0; i-- {
		pipe := p.stops[i-1]
		stack = func(s func(passable P) R, pipe pipeline.Stop[P, R]) func(passable P) R {
